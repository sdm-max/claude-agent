...
function agentsDir(projectPath: string) {
...
// GET /api/projects/[id]/agents — list all agent files
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  const files = listDirectoryFiles(agentsDir(project.path), EXTENSION);
  return NextResponse.json(files);
}

// POST /api/projects/[id]/agents — create a new agent file
export async function POST(req: NextRequest, { params }: Params) {
  ...
  const dir = agentsDir(project.path);
  try {
...
// PUT /api/projects/[id]/agents — update an existing agent file
export async function PUT(req: NextRequest, { params }: Params) {
  ...
  const filePath = path.join(agentsDir(project.path), name);
  if (!fileExists(filePath)) {
...
// DELETE /api/projects/[id]/agents?name=xxx — delete an agent file
