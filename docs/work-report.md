...
## 2026-04-16 세션 3: UX 문제 심화 분석 + 백로그 통합 기획

### 작업 요약
1. Hooks UI 브라우저 테스트 완료 (matcher 체크박스 4건 PASS)
2. 사용자 UX 피드백 기반 심화 분석 — 5개 핵심 문제 발견
3. 백로그 B-5를 최우선으로 승격 및 대폭 확장
4. 4개 분석 에이전트 병렬 투입 → 통합 구현 기획서 작성
5. 전체 커밋 + origin 푸시
...
| # | 테스트 | 결과 |
|---|--------|------|
| 1 | 기존 규칙 뱃지 표시 (PostToolUse Write\|Edit) | PASS |
| 2 | + Rule → 도구 체크박스 11개 그리드 | PASS |
| 3 | 빈 command 저장 차단 | PASS |
| 4 | command 입력 후 저장 | PASS |

### 사용자 피드백 기반 발견 UX 문제

| # | 문제 | 사용자 원문 |
|---|------|------------|
| 1 | JSON 모드에 설명/주석 없음 | "JSON에 주석들이 없네. 어떤게 어떤 코드인지 알 수가 없잖아" |
| 2 | 템플릿 일괄 적용만 가능, 선택 불가 | "어떤건 설정하고 어떤건 설정안하게 가이드를 주고 체크를 해서 적용 해야지" |
| 3 | 적용 후 흔적 없음, Undo 없음 | "카드 적용했으면 거기에 대한 내용을 적용된곳에 표기를 해줘야지" |
| 4 | .sh 파일 빈 에디터, bash 모르면 못 씀 | "sh 파일 생성하면 Hook Wiring처럼 UI로 해야지" |
| 5 | 수동 생성 → 카드 등록 불가, 프로젝트마다 노가다 | "프로젝트마다 일일이 하나하나 노가다 해야 하냐" |
| 6 | deny 설정하면 Claude 잠기고 풀 방법 없음 | "설정했으면 푸는것도 편하게 만들어야지" |

### 백로그 재구성

| 원래 | 통합 후 | 변경 사유 |
|------|---------|----------|
| B-2 (카탈로그) | B-5에 흡수 | 카탈로그 UI = 위자드 프리셋 목록과 동일 구조 |
| B-3 + B-6 | Phase 0 합체 | JSON 설명 + deny 경고 = 같은 settings-form 작업 |
| B-5 | 최우선 승격 + 대폭 확장 | 위자드 + 커스텀 카드 + 삭제 연동 + 3탭 통일 |

### 4개 분석 에이전트 투입 결과

| 에이전트 | 분석 대상 | 핵심 발견 |
|----------|----------|----------|
| 의존관계 분석 | 백로그 6개 항목 | 4-Phase 최적 순서, B-6→B-3 흡수, B-2→B-5 흡수 |
| DB 설계 | 스키마 확장 | 3개 신규 테이블, writeDiskWithSnapshot 반환값 확장 필요 |
| 템플릿 시스템 | 87개 템플릿 구조 | annotateSettingsJson 이미 존재(133키), sections 구조 추가 필요 |
| 공유 컴포넌트 | FileDirectoryEditor 3곳 사용 | AgentEditor는 독자 구현(4탭+2스텝 위자드), Rules/Hooks만 공유 |
| UX 위자드 | 3탭 위자드 설계 | 공통 프레임워크 + 타입별 프리셋/옵션/미리보기, 통합 API 필요 |

### 통합 구현 기획서

`docs/backlog-implementation-plan.md` 신규 작성 (459줄)

| Phase | 내용 | 예상 일수 |
|-------|------|----------|
| 0 | Settings UX 기초 (필드 설명 + deny 경고) | 1~2일 |
| 1 | Scope 추상화 + User 탭 확장 | 3~4일 |
| 2 | 선택적 Apply + Undo + 적용 이력 | 3~4일 |
| 3 | 위자드 + 커스텀 카드 + 카탈로그 통일 | 5~7일 |

### 커밋 목록 (세션 전체)

| 커밋 | 내용 |
|------|------|
| `a5575a0` | fix(hooks): matcher tool selector + empty command validation |
| `c5f2ff5` | fix(layout): wrap Sidebar in Suspense for useSearchParams |
| `df04a69` | docs: scenario 8-10 PASS + backlog B-4~B-6 + gitignore .claude/ |
| `5c836de` | docs: backlog B-5 통합 기획 + 구현 계획서 |
