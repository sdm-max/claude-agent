...
## [B-5] Hooks 수동 편집 UX 강화 (medium)

### 배경
사용자 피드백(2026-04-16): Hooks 탭 Hook Wiring에서 + Rule 누르면 matcher가 빈 자유 텍스트, command도 빈칸. 사용자가 도구 이름을 모르면 유효한 훅을 만들 수 없음. "그냥 빈칸에 글씨만 넣은 거네"

2026-04-16 세션에서 matcher를 도구 체크박스 선택으로 수정 완료 (TOOL_NAMES 상수 + 체크박스 그리드). 빈 command 저장 차단도 추가.

### 추가 개선 필요
- command 입력에 예시/프리셋 제공
- hook type 선택 (현재 command만 지원, http/prompt/agent 미구현)
- 설정 후 실제 동작 테스트 버튼
